...
export default function RestaurantCard({ id, name, address, description, imageUrl }: RestaurantCardProps) {
    const formattedImageUrl = imageUrl 
        ? (imageUrl.startsWith("http") 
            ? imageUrl 
            : `http://localhost:5050${imageUrl.startsWith("/") ? "" : "/"}${imageUrl}`)
        : null;

    return (
        <Link 
            href={`/restaurant/${id}`} 
            className="group block bg-[#0f151d] border border-slate-800/60 rounded-2xl overflow-hidden hover:border-slate-600 transition-all duration-300 shadow-lg hover:shadow-[0_0_30px_-5px_rgba(234,88,12,0.2)] relative"
        >
            <div className="w-full h-56 bg-slate-900 relative overflow-hidden">
                {formattedImageUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-slate-700 font-medium tracking-widest uppercase text-xs">
                        No Image
                    </div>
                )}
            ...
            <div className="p-6 relative">
                <h2 className="text-xl font-medium text-slate-100 tracking-wide mb-1 group-hover:text-orange-500 transition-colors">{name}</h2>
